...
while true do
    ...
    -- Horizontal offset of the menu item.
		x = 5
		if i == j then
			color = red
			x = 10
	...
	if Controls.check(pad, SCE_CTRL_CROSS) then
		...
		Screen.clear()
		Graphics.termBlend()
    -- In microseconds. So .8 seconds.
		System.wait(800000)
		dofile("ux0:/lpp/" .. scripts[i].name)
    -- I guess this is how you'd do the falling edge of a button
    -- press. So, when the up dpad button is released, you go up.
